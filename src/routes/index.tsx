// 1. Função para buscar valor de coluna ignorando caracteres especiais (!, espaços, acentos)

function getRowValue(row: any, targetKey: string): any {
...
// 2. Converter qualquer formato de data (Serial do Excel, String DD/MM/YYYY ou ISO)
...
// 3. Lógica dos Cards no Componente:

let noPrazoCount = 0;

let antecipadoCount = 0;

activeRows.forEach((row) => {

  const incVal = getRowValue(row, 'DataInclusao') || getRowValue(row, 'DataInclusão');

  const carVal = getRowValue(row, 'DataCarregamento');

  const ufVal = (getRowValue(row, 'UF') || '').toString().trim().toUpperCase();

  const dInc = parseToDate(incVal);

  const dCar = parseToDate(carVal);
