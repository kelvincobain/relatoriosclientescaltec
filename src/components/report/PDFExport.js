...
const PDFChartContainer = ({ children, height = 280 }) => (<div style={{ height, width: '100%', position: 'relative' }}>
...
export const PrintOnlyReport = React.forwardRef((props, ref) => {
    ...
    if (!client)
        return null;
    return (<div ref={ref} className="hidden print:block w-[210mm] mx-auto bg-[#0B0F19] text-white p-[15mm] min-h-screen" style={{
      ...
      <header className="flex justify-between items-start border-b border-slate-700 pb-6 mb-8">
        <div className="flex items-center gap-6">
          ...
          <div className="border-l border-slate-700 pl-6">
            ...
            {lastUpdateDate && (<p className="text-[10px] font-medium text-blue-400 mt-0.5">
                ...
                <PDFChartContainer>
                  <BarChart data={monthly} margin={{ top: 35, right: 25, left: 25, bottom: 10 }}>
                    ...
                    <Bar dataKey="tons" fill="#38BDF8" radius={[4, 4, 0, 0]}>
                      <LabelList dataKey="tons" position="top" formatter={(v) => v > 0 ? `${formatNumber(v, 2)}t` : ""} style={{ fontSize: 10, fill: "#94A3B8", fontWeight: 600 }} dy={-8}/>
                    </Bar>
                  </BarChart>
                </PDFChartContainer>
             </ChartCard>
              ...
              <PDFChartContainer>
                <BarChart data={monthly} margin={{ top: 35, right: 25, left: 25, bottom: 10 }}>
                  ...
                  <Bar dataKey={truckKey} fill="#8B5CF6" radius={[4, 4, 0, 0]}>
                    <LabelList dataKey={truckKey} position="top" formatter={(v) => v > 0 ? v : ""} style={{ fontSize: 10, fill: "#94A3B8", fontWeight: 600 }} dy={-8}/>
                  </Bar>
                </BarChart>
              </PDFChartContainer>
            </ChartCard>
              ...
              <PDFChartContainer>
                <BarChart data={otdByMonth} margin={{ top: 35, right: 25, left: 25, bottom: 10 }}>
                  ...
                  <Bar dataKey="rate" radius={[4, 4, 0, 0]}>
                    ...
                    <LabelList dataKey="rate" position="top" formatter={(v) => (v > 0 ? `${formatNumber(v, 1)}%` : "")} fill="#FFFFFF" style={{ fontSize: 10, fontWeight: 700 }} dy={-8}/>
                  </Bar>
                </BarChart>
              </PDFChartContainer>
            </ChartCard>
              ...
              <PDFChartContainer>
                <AreaChart data={dischargeByMonth} margin={{ top: 35, right: 25, left: 25, bottom: 10 }}>
                  ...
                  <Area type="monotone" dataKey="hours" stroke="#F59E0B" strokeWidth={3} fill="#F59E0B" fillOpacity={0.1}>
                    <LabelList dataKey="hours" position="top" formatter={(v) => v > 0 ? `${formatNumber(v, 1)}h` : ""} style={{ fontSize: 10, fill: "#94A3B8", fontWeight: 600 }} dy={-10}/>
                  </Area>
                </AreaChart>
              </PDFChartContainer>
            </ChartCard>
...
export const PDFExportButton = ({ contentRef }) => {
    ...
    return (<Button size="sm" onClick={() => reactToPrintFn()} className="bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/20 transition-all">
