...
export function cancellationStats(
  ...
  for (const row of scoped) {
    const planned = str((row as any)[COL.plannedDelivery as string] || '').split(' ')[0].trim();
    const client = str((row as any)[COL.client as string] || '').trim();
    const city = str((row as any)[COL.city as string] || '').trim();
