...
export function cancellationStats(
  ...
  for (const row of scoped) {
    const planned = str((row as any)[COL.plannedDelivery] != null ? (row as any)[COL.plannedDelivery] : '').split(' ')[0].trim();
    const client = str((row as any)[COL.client] != null ? (row as any)[COL.client] : '').trim();
    const city = str((row as any)[COL.city] != null ? (row as any)[COL.city] : '').trim();
