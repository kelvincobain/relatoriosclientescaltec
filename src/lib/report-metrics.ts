...
export function cancellationStats(
  ...
  for (const row of scoped) {
    const planned = str(row[COL.plannedDelivery as keyof typeof row] || '').split(' ')[0].trim();
    const client = str(row[COL.client as keyof typeof row] || '').trim();
    const city = str(row[COL.city as keyof typeof row] || '').trim();
