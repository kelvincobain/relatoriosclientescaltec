...
export function getServiceTimeData(calRows: Row[], cockpitRows: Row[], selection: Selection): ServiceTimePoint[] {
  ...
  for (const cRow of cockpitRows) {
    ...
    if (selection.client && norm(normalizeClientName(selection.client)) !== norm(normalizeClientName(client))) {
      // DEBUG LOG: console.log(`Mismatch: Sel(${selection.client}) normalized(${normalizeClientName(selection.client)}) vs Row(${client}) normalized(${normalizeClientName(client)})`);
      continue;
    }
