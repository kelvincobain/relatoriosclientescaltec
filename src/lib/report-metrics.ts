...
export function cancellationStats(
  ...
  for (const row of scoped) {
    const planned = str(row[COL.plannedDelivery] != null ? row[COL.plannedDelivery] : '').split(' ')[0].trim();
    const client = str(row[COL.client] != null ? row[COL.client] : '').trim();
    const city = str(row[COL.city] != null ? row[COL.city] : '').trim();
